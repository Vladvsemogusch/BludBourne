...
public class ClockActor extends Label {
    ...
    private void init(){
        String time = String.format(FORMAT, 0, 0, isAfternoon?PM:AM);
        this.setText(time);
        this.pack();
    }
    ...
    public void act(float delta){
        ...
        String time = String.format(FORMAT, hours, minutes, isAfternoon ? PM : AM);
        this.setText(time);
    }

    public int getCurrentTimeSeconds(){
        return MathUtils.floor(totalTime % 60);
    }
    ...
    public int getCurrentTimeHours(){
        int hours = MathUtils.floor((totalTime / 3600) % 24);
